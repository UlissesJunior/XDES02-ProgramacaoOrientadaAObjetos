...
class Tfg:
    ...
    def __init__(self, titulo, aluno, orientador, notaMonografia, notaApresentacao):
        self.__titulo = titulo
        self.__aluno = aluno
        self.__orientador = orientador
        self.notaMonografia = notaMonografia
        self.notaApresentacao = notaApresentacao
    ...
    @property
    def notaMonografia(self):
        return self.__notaMonog
    
    @property
    def notaApresentacao(self):
        return self.__notaApres
    
    @notaMonografia.setter
    def notaMonografia(self, valor):
        if valor < 1 or valor > 10:
            raise ValueError("Valor inválido: {}".format(valor))
        else:
            self.__notaMonog = valor            

    @notaApresentacao.setter
    def notaApresentacao(self, valor):
        if valor < 1 or valor > 10:
            raise ValueError("Valor inválido: {}".format(valor))
        else:
...
class CtrlTfg():       
    ...
    def criaTfg(self, event):
        ...
        notaMonografia = float(self.limiteIns.inputNotaMonog.get())
        notaApresentacao = float(self.limiteIns.inputNotaMonog.get())

        try:
            tfg = Tfg(titulo, aluno, orient, notaMonografia, notaApresentacao)
    ...
    def mostraAprovados(self):
        ...
        for tfg in self.listaTfgs:
            nota = tfg.notaMonografia * 0.7 + tfg.notaApresentacao * 0.3
            if nota >= 6:
                aprovados += f"Título: {tfg.titulo}\nAluno: {tfg.aluno}\nOrientador: {tfg.orientador.nome}\nNota final: {round(nota, 2)}\n----\n"
        if len(aprovados) == 0:
            aprovados = "Não há alunos aprovados"
        self.limiteMostra = LimiteMostraTfgs("TFGs aprovados", aprovados)
    ...
    def mostraReprovados(self):
        ...
        for tfg in self.listaTfgs:
            nota = tfg.notaMonografia * 0.7 + tfg.notaApresentacao * 0.3
            if nota < 6:
                reprovados += f"Título: {tfg.titulo}\nAluno: {tfg.aluno}\nOrientador: {tfg.orientador.nome}\nNota final: {round(nota, 2)}\n----\n"
        if len(reprovados) == 0:
            reprovados = "Não há alunos reprovados"
        self.limiteMostra = LimiteMostraTfgs("TFGs reprovados", reprovados)        